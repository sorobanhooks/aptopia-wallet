...
export const getTokenPrices = async (
  ...
  let sdkPrices: Record<string, { currentPrice: number; percentagePriceChange24h: number | null } | null> = {};
...
export const getIndexerAccountHistory = async ({
  ...
  try {
    ...
    try {
      const url = networkDetails?.networkUrl || "https://horizon-testnet.stellar.org";
...
/**
 * Sign a plain SEP-53 message (the agent-backend SIWE auth challenge) with the
 * active account key. Returns a base64 ed25519 signature the backend verifies.
 */
export const signAuthMessage = async ({
  ...
  const { signature, error } = await sendMessageToBackground({
    message,
    activePublicKey,
    type: SERVICE_TYPES.SIGN_AUTH_MESSAGE,
  });

  if (error || !signature) {
    throw new Error(error || "Failed to sign auth message");
  }
