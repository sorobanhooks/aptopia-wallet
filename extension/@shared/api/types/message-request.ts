...
/**
 * Sign a plain SEP-53 message (not a Stellar transaction). Used by the agent
 * backend SIWE-style auth flow: the popup asks the background service worker to
 * sign the auth challenge string with the active account key.
 */
export interface SignAuthMessageMessage extends BaseMessage {
  type: SERVICE_TYPES.SIGN_AUTH_MESSAGE;
  /** The canonical challenge string returned by /v1/auth/challenge. */
  message: string;
}
