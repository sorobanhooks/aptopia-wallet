...
// The handler delegates contract-metadata fetching to the SDK wallet, which
// validates the contract id and hits Soroban RPC. Stub it so these unit tests
// exercise the store-merge logic with the test's placeholder contract address.
jest.mock("@shared/helpers/stellar", () => ({
  wallet: {
    setNetworkConfig: jest.fn(),
    addCollectible: jest.fn().mockResolvedValue(undefined),
  },
}));
  ...
  it("should add a collectible to an empty store", async () => {
    const result = await addCollectible({
      request: {
        ...
        network: "testnet",
        collectibleContractAddress: "C1234567890",
