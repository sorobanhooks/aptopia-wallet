...
describe("useGetAssetDomainsWithBalances (cached path)", () => {
  ...
  const usdcIssuer = "GCK3D3V2XNLLKRFGFFFDEJXA4O2J4X36HET2FE446AV3M4U7DPHO3PEM";
  const testCanonicalIssuer = TEST_CANONICAL.split(":")[1];

  // useGetBalances serves cache hits from `formattedBalanceData` as a payload
  // whose `balances` is the formatted ARRAY the domains loop iterates over.
  // Keep DT (clean) + USDC (blockaid spam) and no native, so the resolved
  // domain list is exactly those two.
  const cachedPayload = {
    isFunded: true,
    subentryCount: 3,
    error: undefined,
    ...
    balances: [
      ...
      (mockBalances.balances as Record<string, any>)[`USDC:${usdcIssuer}`],
    ],
  };
