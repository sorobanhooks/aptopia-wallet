...
function useGetBalances(_options: BalancesOptions = {}) {
  ...
  const fetchData = useCallback(
    async (
      ...
      // Cache-first: when a cached entry exists for this account, serve it and
      // skip the network round-trip. `cachedBalances` can be undefined when the
      // persisted cache slice predates this field, so guard the lookup.
      const cachedData = cachedBalances?.[network]?.[targetPublicKey];
      if (_useCache && cachedData) {
        if (stateRef.current.state !== RequestState.SUCCESS) {
      ...
      try {
        const horizonServer = new Horizon.Server(
          activeNetworkDetails.networkUrl,
        );
