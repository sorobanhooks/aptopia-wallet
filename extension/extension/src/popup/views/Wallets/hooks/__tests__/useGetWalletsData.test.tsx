...
describe("useGetWalletsData", () => {
  ...
  // useGetBalances now serves cache hits from the `formattedBalanceData` slice
  // (keyed by network then public key) and the stored `balances` is the
  // formatted ARRAY the downstream price/total math iterates over. mockBalances
  // is the old object-keyed shape, so flatten its values into the array the
  // rewritten data layer expects, native-first (as the app sorts balances) so
  // the derived token-price asset id list is [native, DT, USDC].
  const mockBalanceMap = mockBalances.balances as Record<string, unknown>;
  const formattedBalances = [
    mockBalanceMap.native,
    mockBalanceMap[TEST_CANONICAL],
    mockBalanceMap[
      "USDC:GCK3D3V2XNLLKRFGFFFDEJXA4O2J4X36HET2FE446AV3M4U7DPHO3PEM"
    ],
  ];
  const cachedFormattedPayload = {
    isFunded: true,
    subentryCount: 3,
    error: undefined,
    icons: {},
    ...
    const mainnetPreloadedState = {
      ...
      cache: {
        ...
        formattedBalanceData: {
          [MAINNET_NETWORK_DETAILS.network]: {
            ["G1"]: { ...cachedFormattedPayload, publicKey: "G1" },
          },
        },
        icons: cachedIcons,
        tokenLists: tokenListData,
    ...
    const mainnetPreloadedState = {
      ...
      cache: {
        ...
        formattedBalanceData: {
          [MAINNET_NETWORK_DETAILS.network]: {
            ["G1"]: { ...cachedFormattedPayload, publicKey: "G1" },
            ["G2"]: { ...cachedFormattedPayload, publicKey: "G2" },
            ["G3"]: { ...cachedFormattedPayload, publicKey: "G3" },
            ["G4"]: { ...cachedFormattedPayload, publicKey: "G4" },
            ["G5"]: { ...cachedFormattedPayload, publicKey: "G5" },
            ["G6"]: { ...cachedFormattedPayload, publicKey: "G6" },
            ["G7"]: { ...cachedFormattedPayload, publicKey: "G7" },
          },
        },
        icons: cachedIcons,
        tokenLists: tokenListData,
