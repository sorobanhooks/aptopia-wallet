...
describe("AddCollectibles", () => {
  ...
  it("shows 'Collectible not found' error when fetchedCollectibles returns an error", async () => {
    // The view's submit handler delegates the collectible lookup/add to the
    // `addCollectible` internal API (SDK wallet -> Soroban RPC). When the
    // contract/token can't be resolved it returns `{ error }`, which the view
    // renders. Stub it here so the "not found" path is exercised without RPC.
    jest
      .spyOn(internalApi, "addCollectible")
      .mockResolvedValue({ error: "Collectible not found" } as any);
  ...
  it("shows 'Collectible not found' error when token ID is not found in collection.collectibles", async () => {
    // The view delegates lookup/add to `addCollectible`; a token ID absent from
    // the collection surfaces as `{ error: "Collectible not found" }`.
    jest
      .spyOn(internalApi, "addCollectible")
      .mockResolvedValue({ error: "Collectible not found" } as any);
  ...
  it("shows 'Collectible not found' error when contract address does not match any collection", async () => {
    // The view delegates lookup/add to `addCollectible`; a contract that matches
    // no collection surfaces as `{ error: "Collectible not found" }`.
    jest
      .spyOn(internalApi, "addCollectible")
      .mockResolvedValue({ error: "Collectible not found" } as any);
